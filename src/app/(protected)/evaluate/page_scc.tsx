...
import { ScoreClimberCard } from "@/app/_components/ScoreClimberCard";
...
// Finds a point that was missed on the first attempt but is no longer missed —
// this becomes the "gap closed" line on the Score Climber card.
function computeGapClosed(firstMissed: string[], latestMissed: string[]): string {
  const closed = (firstMissed ?? []).find((point) => !(latestMissed ?? []).includes(point));
  return closed ?? "Overall answer structure and clarity improved";
}
...
export default function EvaluatePage() {
  ...
  const [attemptHistory,    setAttemptHistory]    = useState<
    { marks_awarded: number; total_marks: number; points_missed: string[]; created_at: string }[]
  >([]);
  ...
  useEffect(() => {
    if (!questionNumber) { setSelectedQuestion(null); setAttemptHistory([]); return; }
    const q = questions.find((q) => q.question_number === questionNumber) ?? null;
    setSelectedQuestion(q);
    setStudentAnswer(""); setResult(null); setError(null); setAttemptHistory([]);
    if (q) {
      ...
      // Fetch prior attempts on this exact question — drives repeat-attempt pricing
      // (2 tokens instead of 3) and the Score Climber comparison after submit.
      if (userId) {
        supabase
          .from("evaluations")
          .select("marks_awarded, total_marks, points_missed, created_at")
          .eq("user_id", userId)
          .eq("question_id", q.id)
          .order("created_at", { ascending: true })
          .then(({ data }) => {
            setAttemptHistory((data ?? []) as typeof attemptHistory);
          });
      }
    }
  }, [questionNumber, questions, userId]);
  ...
  const isRepeatAttempt = attemptHistory.length > 0;
        ...
        {selectedQuestion && (
          <div className={`${mutedPanel} mt-6 space-y-3`}>
            <div className="flex items-center justify-between gap-3">
              ...
              {isRepeatAttempt && (
                <span className="rounded-full border border-emerald-800 bg-emerald-950/30 px-2.5 py-0.5 text-xs font-medium text-emerald-400">
                  Attempt {attemptHistory.length + 1} · 2 tokens
                </span>
              )}
            </div>
          ...
          <div className="space-y-8">
            ...
            /* Subjective */
              ...
              <>
                ...
                {/* Score Climber — only when this attempt beat the first attempt on this question */}
                {attemptHistory.length > 0 && result.marks_awarded > attemptHistory[0].marks_awarded && (
                  <ScoreClimberCard
                    subject={subject}
                    questionRef={`${subject} · Q${selectedQuestion?.question_number} · ${year}`}
                    attempt1Score={attemptHistory[0].marks_awarded}
                    latestScore={result.marks_awarded}
                    totalMarks={result.total_marks}
                    gapClosed={computeGapClosed(attemptHistory[0].points_missed, result.points_missed)}
                  />
                )}
