...
function validateEntry(entry) {
  const required = ['question_number', 'question_text', 'is_subjective', 'question_type'];
  // JSON may use 'marks' (PPA prompt output) or 'total_marks' (older scripts) — accept either
  if (entry.marks === undefined && entry.total_marks === undefined) {
    required.push('marks');
  }
...
async function importFile(jsonPath, subjectId) {
  ...
  const marking_schemes = data.marking_schemes ?? data.questions;

  if (!year || !Array.isArray(marking_schemes) || marking_schemes.length === 0) {
    console.error(`   ❌ JSON missing 'year' or a 'marking_schemes'/'questions' array.`);
    return { inserted: 0, skipped: 0, failed: 0, warnings: 0 };
  }
      ...
      if (existing && (!existing.marking_schemes || existing.marking_schemes.length === 0)) {
        ...
        const { error: msErr } = await supabase
          ...
          .insert({
            ...
            model_answer_verified: entry.model_answer_verified ?? false,
