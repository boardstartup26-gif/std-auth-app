...
- **Parental consent gates evaluation (DPDP Act), after a 2-evaluation free quota.** `POST /api/evaluate` calls `getEvaluationGateState()` ([src/lib/parent-consent/service.ts](src/lib/parent-consent/service.ts)) right after auth and refuses unless `gate.allowed` — *before* the question lookup, credit reservation, persistence, or the Anthropic call. Keep that ordering: moving the check later means an unconsented minor's answer gets processed. `getEvaluationGateState()` wraps `getParentConsentState()` with `FREE_EVALUATIONS_BEFORE_CONSENT` (currently 2, counted from `student_answers` rows) — a new student may complete that many evaluations regardless of consent status, and only from the next one on does `parent_consents.status = 'confirmed'` become mandatory. An explicit withdrawal (`status = 'revoked'`) always blocks immediately, even inside the free quota — it is the one state the quota never overrides. Any new route that stores or sends student answers needs the same `getEvaluationGateState()` check, not the bare consent check. The `/evaluate` layout lock and the banner in `(protected)/layout.tsx` are UX only, and the banner now speaks in two tiers (soft nudge while free evaluations remain, hard "locked" once they're used) — both driven by the same `gate.allowed`, so don't let them drift from the API's own decision. Withdrawal is a manual SQL action for now — the runbook is at the bottom of [supabase/migrations/20260917120000_parent_consents.sql](supabase/migrations/20260917120000_parent_consents.sql). Consent links are signed with `PARENT_CONSENT_TOKEN_SECRET` and built from `NEXT_PUBLIC_SITE_URL`/`SITE_URL` via [src/lib/site-url.ts](src/lib/site-url.ts), never from the request host.
  - **`DISABLE_PARENT_CONSENT_GATE=true` is a *separate*, unconditional production kill switch** (added 2026-09-18, no working outbound email at the time), checked inside `getParentConsentState()` itself, so `getEvaluationGateState()` and everything built on it inherit it automatically. It treats every non-revoked user as confirmed without touching the underlying `parent_consents` row, which makes the free-quota logic above moot while it's on — everyone passes, not just the first 2. **This should not be on in production once `EMAIL_FROM`/`RESEND_API_KEY` are working** — it was a stopgap for a broken email pipeline, not a supported mode, and the free-quota mechanism above is the actual long-term design. Check whether it's still set before assuming the 2-evaluation quota is what's actually governing access.
- **Welcome emails go to the student, only on a genuine new signup, never a login.** `sendWelcomeEmail()` ([src/lib/email/welcome.ts](src/lib/email/welcome.ts)) is called from `signup()` in [src/app/(auth)/actions.ts](src/app/(auth)/actions.ts) and from the Google branch of [src/app/auth/callback/route.ts](src/app/auth/callback/route.ts), both already gated on `isRealNewUser`/`isNewAccount` for their own consent-recording logic — reuse that same flag rather than adding a new check. It is unrelated to the parent-consent email (`requestParentConsent()` in the same service file), which goes to the parent's address instead.
