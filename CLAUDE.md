...
- **Parental consent gates evaluation (DPDP Act).** `POST /api/evaluate` calls `getParentConsentState()` ([src/lib/parent-consent/service.ts](src/lib/parent-consent/service.ts)) right after auth and refuses unless `parent_consents.status = 'confirmed'` — *before* the question lookup, credit reservation, persistence, or the Anthropic call. Keep that ordering: moving the check later means an unconsented minor's answer gets processed. Any new route that stores or sends student answers needs the same check. The `/evaluate` layout lock and the banner in `(protected)/layout.tsx` are UX only. Withdrawal is a manual SQL action for now — the runbook is at the bottom of [supabase/migrations/20260917120000_parent_consents.sql](supabase/migrations/20260917120000_parent_consents.sql). Consent links are signed with `PARENT_CONSENT_TOKEN_SECRET` and built from `NEXT_PUBLIC_SITE_URL`, never from the request host.
  - **`DISABLE_PARENT_CONSENT_GATE=true` is a temporary production kill switch** (added 2026-09-18, no working outbound email at the time), checked inside `getParentConsentState()` itself, so all three consumers above pick it up automatically. It treats every non-revoked user as confirmed without touching the underlying `parent_consents` row — an explicit withdrawal (`status = 'revoked'`) is the one state it never overrides. **This should not be on in production for longer than it takes to get `EMAIL_FROM`/`RESEND_API_KEY` working** — it is a stopgap for a broken email pipeline, not a supported mode. Check whether it's still set before assuming the gate is actually enforcing anything.
